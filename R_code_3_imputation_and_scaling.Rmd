...
# Adding and Deleting some variables
```{r}
data <- data %>%
  rowwise() %>%  # This ensures that the summing is done row by row
  mutate(appCat_total = sum(c_across(starts_with("appCat.")), na.rm = TRUE)) %>%
  ungroup()
...
data_TEST <- data %>% filter(ID %in% test_participants)
unique(as.numeric(data_TEST$ID))

`%not_in%` <- purrr::negate(`%in%`)
data_TRAIN <- data %>% dplyr::filter(ID %not_in% test_participants)
unique(as.numeric(data_TRAIN$ID))
```

```{r}
# data_to_write_TRAIN <- fix_time(data_TRAIN_id)
write.csv(data_TRAIN, "data/data_2_TRAIN_ID_aggregated_beep.csv")

# data_to_write_TEST <- fix_time(data_TEST_id)
write.csv(data_TEST, "data/data_2_TEST_ID_aggregated_beep.csv")
```
...
data <- data %>%
  ...
  ) %>% select(-mean, -sd, -SUM_inactivity_timediff)
```
...
```{r}
mean_mood_train <- mean(imputed_data_TRAIN$mood, na.rm = TRUE)
mean_valence_train <- mean(imputed_data_TRAIN$valence, na.rm = TRUE)
mean_arousal_train <- mean(imputed_data_TRAIN$arousal, na.rm = TRUE)
mean_activity_train <- mean(imputed_data_TRAIN$activity, na.rm = TRUE)

data <- data %>% mutate(
  mood = if_else(is.na(mood), mean_mood_train, mood),
  valence = if_else(is.na(valence), mean_valence_train, valence),
  arousal = if_else(is.na(arousal), mean_arousal_train, arousal),
  activity = if_else(is.na(activity), mean_activity_train, activity),
  )
```
...
```{r}
imputed_data_TEST <- data
```



```{r}
write.csv(imputed_data_TEST,"data/test.csv")
write.csv(imputed_data_TRAIN,"data/train.csv")
...
```{r}
data$arousal <- factor(data$arousal, ordered = T)
data$arousal
data$valence <- factor(data$valence, ordered = T)
