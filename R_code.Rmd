...
```{r}
id <- data %>% filter(ID == 17)

id_2_days <- id %>% filter(days_since_start == 0 | days_since_start == 1)


unique(id$days_since_start)
```
...
data <- data %>% group_by(ID) %>% mutate(
  ...
  possible_sleep_gap = (time_diff >= 6 & time_diff <= 15) & 
...
data <- data %>% group_by(ID) %>% mutate(
    consecutive_inactive = with(rle(is_inactive), {
      lengths_corrected <- lengths * values  # Only count lengths where is_inactive is TRUE
      rep(lengths_corrected, lengths)   # Repeat these lengths according to run lengths
    })) %>% 
  ungroup()
```
...
data <- data %>% group_by(ID)%>% mutate(
    sleep_start = (
      consecutive_inactive >= 4 & 
        lag(consecutive_inactive, default = 0) < 4) | 
      lag(possible_sleep_gap, default = FALSE),
    sleep_end = (
      consecutive_inactive >= 4 & 
        lead(consecutive_inactive, default = 0) < 4) | 
      possible_sleep_gap,
    session_id = cumsum(sleep_start)
  )

unique(data$session_id)
```
...
data_summarized <- data %>%
  ...
  summarize(
    bedtime = if(any(sleep_start)) min(date_time[sleep_start], na.rm = TRUE) else NA_real_,
    wake_time = if(any(sleep_end)) max(date_time[sleep_end], na.rm = TRUE) else NA_real_,
    .groups = 'drop'
  ) %>%
  ungroup() %>%
  mutate(
    # Ensure that NA values are returned as NA of type POSIXct to maintain type consistency
    bedtime = as.POSIXct(bedtime, origin = "1970-01-01", tz = "UTC"),
    wake_time = as.POSIXct(wake_time, origin = "1970-01-01", tz = "UTC")
  ) %>%
  ungroup()
```
...
data <- data %>% group_by(ID)%>%
  mutate(
    # Detect start of sleep when there are at least 4 consecutive inactive periods
    sleep_start = consecutive_inactive >= 4 & lag(consecutive_inactive, default = 0) < 4,
    # Detect end of sleep by change from inactive to active
    sleep_end = !is_inactive & lag(is_inactive, default = FALSE)
  )


data <- data %>% group_by(ID, date) %>% mutate(
    consecutive_inactive = with(rle(is_inactive), {
      lengths_corrected <- lengths * values  # Only count lengths where is_inactive is TRUE
      rep(lengths_corrected, lengths)   # Repeat these lengths according to run lengths
    }))
...
y <- c(TRUE,TRUE,TRUE,FALSE,FALSE,TRUE,TRUE,TRUE,FALSE)

with(rle(x),rep(lengths, lengths))
table(data$consecutive_inactive)
```
...
data <- data %>% group_by(ID) %>%
  mutate(
    time_diff = round(
      difftime(date_time, 
               lag(date_time, default = first(date_time)),
               units = "hours"), 3),
    cum_sum_activity = cumsum(is_inactive),
    lag_12 = lag(cum_sum_activity, 12, default = first(cum_sum_activity)),
    lag_12_activity = cum_sum_activity - lag_12,
    is_asleep = lag_12_activity >= 4
    ) %>%
  ungroup() %>%
  mutate(
    # Adjust the date to the previous day if the activity is after midnight and before going to sleep
    adjusted_date_time = if_else(hour(date_time) < 6 & !is_asleep,
                                 date_time - days(1), 
                                 date_time),
...
data <- data %>% mutate(
  cum_sum_activity = cumsum(is_inactive),
  lag_12 = lag(is_yarrak, 12), # maximum amount of sleep hours
  lag_12_activity = cum_sum_activity - lag_12, 
  is_asleep = is_yarrak_3 >= 4) # minimum amount of sleep hours
...
data <- data %>%
  mutate(
    ...
    change_id = cumsum(changed != lag(changed, default = first(changed)))  
    # Detect changes to start new blocks
  ) %>%
...
id_1 <- data %>% filter(ID == 1)
write.csv(id_1, "x.csv")
```
