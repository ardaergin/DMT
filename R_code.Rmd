...
```{r}
participants <- unique(data_raw$id)
set.seed(17042024)
test_participants <- sample(participants, length(participants)* 1/4)

test_set <- data_1 %>% filter(ID %in% test_participants)

`%not_in%` <- purrr::negate(`%in%`)
train_set <- data_1 %>% dplyr::filter(ID %not_in% test_participants)
unique(as.numeric(train_set$ID))
```
...
# > 4000
data_filter_time_2 <- data %>% filter(
...
write.csv(data_to_write, "data_1.csv")
```
...
```{r}
write.csv(aggregated_data, "data_2_aggregated.csv")
```
