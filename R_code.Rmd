...
aggregated_night_data <- data_beepo_0 %>%
  ...
  summarise(
    ...
    max_time_diff = max(time_diff_valid),
    
    total_inactivity = safe_sum(consecutive_inactive_fix),
    max_inactivity = max(consecutive_inactive_fix),
...
```{r}
night_mood_replacement <- data_with_night %>%
  arrange(ID, observation_day, beepo) %>%
  group_by(ID) %>%
  mutate(
    # Create lagged versions of night variables to fill backwards
    lead_night_mood = if_else(
      beepo == 5, lead(night_mood, default = first(night_mood)), NA_real_),
    lead_night_arousal = if_else(
      beepo == 5, lead(night_arousal, default = first(night_arousal)), NA_real_),
    lead_night_valence = if_else(
      beepo == 5, lead(night_valence, default = first(night_valence)), NA_real_)
  ) %>% 
  mutate(
    mood = case_when(
